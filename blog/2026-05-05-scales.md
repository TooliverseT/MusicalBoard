...
In [Vocal Scales](https://www.musicalboard.com/vocal-scales/), select the "Major" preset and set the BPM somewhere between 60 and 70. Press play: you get a four-beat count-in (with the first root on count 1), then the pattern runs with a click and piano on every step. Watch **Root**, **Now**, and **Next** in the status row — **Now** shows the pitch you should sing (or count/gap labels between keys) and the active step lights up in the pattern strip. Go all the way up and back down as one continuous phrase, listening for any wobble on individual notes. Keep [Vocal Pitch Monitor](https://www.musicalboard.com/vocal-pitch-monitor/) open in another tab — seeing how much your pitch drifts above and below the target note in real time makes it much easier to catch problem spots. Do two passes on each key, then move up a half step.
...
<p align="center">
  <em>Using round-trip direction with a low BPM lets you move through the passaggio slowly instead of skipping over it. Between chromatic keys, a short key gap (clicks plus the next root preview) gives you a moment to reset before the new scale starts.</em>
</p>
...
[Vocal Scales](https://www.musicalboard.com/vocal-scales/) lets you set the root range, BPM, and direction (ascending, descending, or both), and switch between major, minor, pentatonic, blues, and modal presets in a single click. You can also build a custom **pattern** in the strip — ordered degrees, duplicates allowed — not only fixed up-and-down shapes. Piano samples play each step on the beat (with a built-in click, separate from the Metronome page), and a count-in plus a four-beat **key gap** between chromatic roots help you sing in time when the key changes. **Root**, **Now**, and **Next** tell you which key you're in, what to sing now, and what's coming up.
